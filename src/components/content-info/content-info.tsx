...
const ContentInfo = (contentInfo: IContentInfo) => {
  return (
    <div className="relative w-full">
      <div className="relative w-full">
        ...
        <Image
          src={contentInfo.heroBackground}
          alt="Hannibal"
          className="w-full hidden md:block md:h-[580px]"
        />
        <Image
          src={contentInfo.mobileHeroBackground}
          alt="Hannibal"
      ...
      <div className="absolute max-md:translate-y-[-10px] md:inset-0 w-full flex items-end md:items-center justify-center md:justify-start px-4 md:px-32 pb-8 md:pb-0">
        <div className="w-full relative space-y-4 md:space-y-6 text-center md:text-right">
          <Image
            src={contentInfo.cover}
            width={380}
            alt={'cover'}
            className="hidden md:block"
          ></Image>
          <h1 className="text-sm md:text-lg font-bold">{contentInfo.title}</h1>
          ...
          <div className="text-gray-300 text-xs flex flex-wrap justify-center md:justify-start items-center gap-2">
            <div className="flex items-center gap-1">
              ...
              <span>%{contentInfo.extras.likes}</span>
            </div>
            <span>·</span>
            <div className="flex items-center gap-1">
              <Image src={imdb} alt={'imdb'}></Image>
              <span>{contentInfo.extras.imdb}</span>
            </div>
            <span>·</span>
            <span className="whitespace-nowrap">{contentInfo.extras.duration}دقیقه</span>
            <span>·</span>
            <span className="whitespace-nowrap">{contentInfo.extras.productionTime}</span>
            <span>·</span>
            <span>{contentInfo.extras.country}</span>
            <span>·</span>
            <span className="whitespace-nowrap">{contentInfo.extras.seasons}</span>
            <span>·</span>
            <span className="whitespace-nowrap">{contentInfo.extras.translation}</span>
            <span>·</span>
            <span className="whitespace-nowrap">{contentInfo.extras.age}</span>
          </div>

          <div className="flex justify-center md:justify-start gap-2">
            {contentInfo.extras.genres.map((tag, index) => (
              <div key={index}>
                <Tag label={tag} />
              </div>
            ))}
          ...
          <div className="md:max-w-[580px] text-right">
            <span className="text-xs md:text-sm">{contentInfo.description}</span>
          </div>

          <div className="hidden md:block absolute left-[-50px] bottom-10">
            <Image src={contentInfo.channel} alt={'channel'} width={85}></Image>
          </div>
        </div>
      </div>
    </div>
